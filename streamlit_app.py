            ...
            if st.button("📤 Upload to Supabase", type="primary"):
                with st.spinner("Uploading to Supabase..."):
                    ...
                    if success:
                        ...
                        # Show RLS policy help if that's the issue
                        if "row level security" in message.lower() or "rls" in message.lower():
                            st.warning("""
                            **🔒 Row Level Security (RLS) Issue Detected**
                            
                            Your Supabase `deals` table has Row Level Security enabled, which is blocking the upload.
                            
                            **To fix this, you need to:**
                            1. Go to your Supabase dashboard
                            2. Navigate to Authentication → Policies
                            3. Find the `deals` table
                            4. Either disable RLS or add a policy that allows inserts
                            
                            **Quick Fix:** Add this policy to allow all operations:
                            ```sql
